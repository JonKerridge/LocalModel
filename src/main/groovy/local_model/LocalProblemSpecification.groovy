...
class LocalProblemSpecification {
  /**
   ...
   * @param replaceNumber the number of individuals created during a replacement operation
   * @repalceInterval number of generations after which a replacement will be undertaken when there
   * has been no change in the best fitness value
  ...
  int crossoverPoints, maxGenerations, replaceNumber, replaceInterval
   ...
   */
  LocalProblemSpecification copySpecification(){
    LocalProblemSpecification ps = new LocalProblemSpecification()
    ...
    ps.replaceNumber = replaceNumber
...
//        "$mutationProbability, $convergenceLimit, $minOrMax, $seeds"
    return "$dataFileName, $nodes, $populationPerNode, $geneLength, " +
        ...
        "$mutationProbability, $convergenceLimit, $maxGenerations, $replaceNumber, $replaceInterval"
  }
